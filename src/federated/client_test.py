...
class FlowerClient(fl.client.NumPyClient):
    ...
    def evaluate(
        ...
        if self.run_id:
            try:
                ml_client = MlflowClient(tracking_uri="https://mlflow-server-789083630799.us-central1.run.app")
                ml_client.log_metric(self.run_id, f"client_{self.center_id}_val_loss", avg_loss)
                ml_client.log_metric(self.run_id, f"client_{self.center_id}_dice", dice_score)
            except Exception as e:
                print(f"⚠️ MLflow logging failed: {e}")
